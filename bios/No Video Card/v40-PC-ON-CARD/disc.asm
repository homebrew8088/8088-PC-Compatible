...
INITIALIZE_CH376S_0XE0:
	...
	MOV CX, END_DEBUG_TEXT - DEBUG_TEXT
    MOV BX, DEBUG_TEXT
    CALL WRITE_MESSAGE;
	%endif

	MOV CX, END_CHECKPOINT_2 - CHECKPOINT_2
    MOV BX, CHECKPOINT_2
    CALL WRITE_MESSAGE;
...
; V20/30/40 will not set the zero flag on multiply
...
; 8086 will wrap around a word write, so writing to
; 2000:FFFF will end up at 2000:0000 instead of 2001:0000
	PUSH DS
	MOV AX, 0x2001  ; Initialize 2001:0 to zero
	MOV DS, AX
	MOV WORD [DS:0], 0

	MOV AX, 0x2000	; Try a write at 2000:FFFF
	MOV DS, AX
	MOV WORD [DS:0xFFFF],0x9999

	MOV AX, 0x2001	; Read 2001:0000
	MOV DS, AX
	MOV AL, [DS:0]
	POP DS
	CMP AL, 0x99	; If it went over, we have a 186
	JE HAS_186

	MOV CX, END_8086_TEXT - BEGIN_8086_TEXT
    MOV BX, BEGIN_8086_TEXT
    CALL WRITE_MESSAGE;
	MOV DX, INT13_8086	; Worst case, assume 8086
	JMP CPU_DECIDED

HAS_186:
	MOV CX, END_186_TEXT - BEGIN_186_TEXT
    MOV BX, BEGIN_186_TEXT
    CALL WRITE_MESSAGE;
	MOV DX, INT13
	JMP CPU_DECIDED

HAS_V40:
	MOV CX, END_V40_TEXT - BEGIN_V40_TEXT
    MOV BX, BEGIN_V40_TEXT
    CALL WRITE_MESSAGE;
	MOV DX, INT13
	JMP CPU_DECIDED
...
CH376S_WRONG_MODE:
	...
	MOV CX, END_WRONG_MODE_MSG2 - WRONG_MODE_MSG2
    MOV BX, WRONG_MODE_MSG2
    CALL WRITE_MESSAGE
	CALL END_OF_LINE
...
NO_DISC_MSG:
	DB 0x0D, 0x0A, 0x15, ' No drive reported by module.  Skipping install.', 0x0D, 0x0A
END_NO_DISC_MSG:

NO_CH376S_MSG:
	DB 0x0D, 0x0A, 0x15, ' CH376S Module does not respond to existence check.  Skipping install.', 0x0D, 0x0A
END_NO_CH376S_MSG:

WRONG_MODE_MSG:
	DB 0x15, ' CH376S failed switching USB mode - response code '
END_WRONG_MODE_MSG:

WRONG_MODE_MSG2:
	DB 0x15, '.  Skipping install.', 0x0D, 0x0A
END_WRONG_MODE_MSG2:


CHECKPOINT_1:
    DB 'Standalone CH376S Firmware v0.52 - 2021-12-07', 0x0D, 0x0A
END_CHECKPOINT_1:

CHECKPOINT_2:
    DB 0xF9, ' Resetting Module...'
END_CHECKPOINT_2:

DEBUG_TEXT:
	DB 0xF9, ' Debug Mode: CH376S errors will be displayed at runtime', 0x0D, 0x0A
END_DEBUG_TEXT:

MODE_SET_TEXT:
    DB 0x0D, 0x0A, 0xF9, ' Setting USB mode...'
END_MODE_SET_TEXT:

REVINFO:
	DB 0x0D, 0x0A, 0xF9, ' IC Revision: '
END_REVINFO:

DEVINFO:
	DB 0x0D, 0x0A, 0xF9, ' Device ID: '
END_DEVINFO:

BEGIN_8086_TEXT:
	DB 0x0D, 0x0A, 0xF9, ' 8086-class CPU detected, using 8086-compatible disc routines.', 0x0D, 0x0A
END_8086_TEXT:

BEGIN_186_TEXT:
	DB 0x0D, 0x0A, 0xF9, ' 186+ class CPU detected, using 186-compatible disc routines.', 0x0D, 0x0A
END_186_TEXT:

BEGIN_V40_TEXT:
	DB 0x0D, 0x0A, 0xF9, ' NEC V20/30/40 CPU detected, using 186-compatible disc routines.', 0x0D, 0x0A
END_V40_TEXT:
...
INT13_8086:
  ...
  .START_INT13:	
	...
	JE GET_STATUS_LAST_OPERATION	;GET STATUS OF LAST OPERATION 
	CMP AH, 0X02	
	JE DISK_READ_8086				;READ DISK CHS
	CMP AH, 0X03
	JE WRITE_DISK_8086				;WRITE DISK CHS
	CMP AH, 0X08
	JE PARAMETERS					;GET DISK PARAMETERS
	CMP AH, 0X15
	JE GET_DISK_TYPE				;GET DISK TYPE
	...
	JE PLACEHOLDER_RETURN			;Calibrate Drive
	CMP AH, 0X05
	JE FORMAT_TRACK_8086			;FORMAT TRACK
	CMP AH, 0X06
	JE PLACEHOLDER_RETURN			;FORMAT TRACK/MARK BAD
	CMP AH, 0X04
	JE PLACEHOLDER_RETURN			;VERIFY
	CMP AH, 0X0C
	JE SEEK_8086					;Seek to cylinder
	CMP AH, 0X12
	JE PLACEHOLDER_RETURN			;Controller Diagnostic
	CMP AH, 0X13
	JE PLACEHOLDER_RETURN			;Drive Diagnostic
...
%endif	
	...
	MOV DX, DATA_PORT				;SET PORT NUMBER
...
DISK_READ_8086:
	...
	CALL DISK_IN_BOUND_8086	; Clobbers disk number if outside of range
	CMP DL, 0x99
	JNE .REQUESTED_DETAILS_SANE
	MOV AL, 0x99
	JMP READ_DISK_ERROR
...
.READ_IN_LOOP:		    ;LOOP LABLE
	...
	MOV AL, CH376S_DISK_RD_GO		;DISK_RD_GO READ NEXT 64 BYTES
	OUT COMMAND_PORT, AL			;OUT COMMAND PORT
	JMP .READ_SECTOR_8086				;LOOP UNTIL DONE
...
%endif	
	...
	MOV CX, AX							;SET CX TO NUMBER OF BYTES
	MOV DX, DATA_PORT					;SET PORT NUMBER
	MOV SI, BX							;SET TARGET DESTINATION
	CLD									;SET DIRECTION
	REP OUTSB							;BULK STORE
	ADD BX, AX							;Bump BX one batch over
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;WRITE DISK SECTOR(S) - 8086 version
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;LBA = (C × HPC + H) × SPT + (S − 1)
;MAX NUMBERS C = 0X3FF, H = 0XFF, S = 0X3F
;AH = 03h
...
%endif	
 ...
 .WRITE_IN_LOOP:		;LOOP LABLE
	...
	MOV AL, CH376S_DISK_WR_GO			;DISK_WR_GO WRITE NEXT 64 BYTES
	OUT COMMAND_PORT, AL				;OUT COMMAND PORT
	JMP .WRITE_SECTOR_8086					;LOOP UNTIL DONE
...
FORMAT_TRACK_8086:
	...
	AND CL, 0b11000000				;Remove the "sector" part of CX (ignored) and
	OR CL, 1						;treat it as "Sector 1", and
									;the CHS->LBA formula will get us the first block of the track

	CALL DISK_IN_BOUND_8086
	CMP DL, 0x99
	JNE .REQUESTED_DETAILS_SANE
	MOV AL, 0x99
	JMP WRITE_DISK_ERROR
...
.FORMAT_BYTE_8086:
	OUT DATA_PORT, AL
	LOOP .FORMAT_BYTE_8086
	MOV AL, CH376S_DISK_WR_GO			;DISK_WR_GO READ NEXT 64 BYTES
	OUT COMMAND_PORT, AL				;OUT COMMAND PORT
	JMP .FORMAT_SECTOR_8086					;LOOP UNTIL DONE
...
.SAFE_PARAMS:
	...
	SHL AH,1				; Always use 8086 instructions because
	SHL AH,1				; this is not performance critical
	SHL AH,1
	SHL AH,1
	SHL AH,1
	SHL AH,1
...
WRITE_SECTORS_IN_MB:
	...
	SHR AX,1  ; This remains 8086 version because it's not performance critical
	SHR AX,1
	SHR AX,1
	SHR AX,1
	SHR AX,1
...
OVERSIZE_MESSAGE:
	DB 0x0D, 0x0A, 0x04, ' Device size limited to '
END_OVERSIZE_MESSAGE:

BELOW_TABLE_SIZE_MESSAGE:
	DB 0x0D, 0x0A, 0x1A, ' Device smaller than ROM parameter table.  Suggested cylinder count: 0x'
...
CONVERT_CHS_TO_LBA:
	...
	SHR CL, 0X06			;SHIFT RIGHT 6 TIMES FOR UPPER 2 BITS CYLINDER
...
DISK_IN_BOUND_8086:
	...
	CALL CONVERT_CHS_TO_LBA_8086; DX:AX has requested LBA value for first sector
	POP BX
	CMP BH, 0x05	
	JNE NOT_IN_FORMAT_8086
	MOV BL, MAX_SPT
NOT_IN_FORMAT_8086:
	DEC BL
	MOV BH, 0
	ADD AX, BX					; Adding BL (or MAX_SPT for format) gives last sector read/wrote
